# entropy_check.py — measure actual entropy distribution on instruct model
from datasets import load_dataset
from generate_instruct import format_instruct_prompt, get_layer_logits_cached, compute_entropy
import numpy as np
from generate_instruct import tokenizer, model

dataset = load_dataset("truthful_qa", "generation", split="validation")

entropies = []
for i, sample in enumerate(dataset.select(range(20))):
    prompt = format_instruct_prompt(sample["question"])
    input_ids = tokenizer.encode(prompt, return_tensors="pt").to(model.device)
    layer_logits, _ = get_layer_logits_cached(input_ids, None)
    H = compute_entropy(layer_logits[-1])
    entropies.append(H)
    print(f"  Q{i+1}: entropy={H:.3f} | {sample['question'][:50]}")

print(f"\nMean entropy: {np.mean(entropies):.3f}")
print(f"Max entropy:  {np.max(entropies):.3f}")
print(f"Min entropy:  {np.min(entropies):.3f}")
print(f"% above 3.5:  {np.mean(np.array(entropies) > 3.5):.1%}")
print(f"% above 2.0:  {np.mean(np.array(entropies) > 2.0):.1%}")
print(f"% above 1.0:  {np.mean(np.array(entropies) > 1.0):.1%}")
print(f"% above 0.5:  {np.mean(np.array(entropies) > 0.5):.1%}")
