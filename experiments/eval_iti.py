...
configs = [
    # Low-alpha ITI tends to avoid the severe degradation seen at 10-20.
    ("ITI alpha=0.5",
     lambda q: iti_generate(format_instruct_prompt(q), alpha=0.5)),

    ("ITI alpha=1.0",
     lambda q: iti_generate(format_instruct_prompt(q), alpha=1.0)),

    ("ITI alpha=2.0",
     lambda q: iti_generate(format_instruct_prompt(q), alpha=2.0)),
]
...
if __name__ == "__main__":
    ...
    for label, fn in configs:
        ...
        out_path = ROOT / "results" / "iti_results.json"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(all_results, f, indent=2)
        print(f"  Saved to results/iti_results.json", flush=True)
    ...
    print("Full results saved to results/iti_results.json", flush=True)
